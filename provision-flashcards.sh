...
until docker exec -i flashcards.db mysql -u root -se "SELECT EXISTS(SELECT 1 FROM mysql.user WHERE user = 'root')" &> /dev/null
do
  printf "."
  sleep 1
done
...
# Create the database
docker exec -i flashcards.db mysql -u root -se "CREATE DATABASE flashcards;"

# Run migrations
echo -e "${GREEN}Running migrations for ${name}...${NC}"
docker exec -t flashcards.app bash -c "cd /edx/app/${name}/ && make migrate"

# Create superuser
echo -e "${GREEN}Creating super-user for ${name}...${NC}"
docker exec -t flashcards.app bash -c "echo 'from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.create_superuser(\"edx\", \"edx@example.com\", \"edx\") if not User.objects.filter(username=\"edx\").exists() else None' | python /edx/app/${name}/manage.py shell"
